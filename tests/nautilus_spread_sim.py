"""
Nautilus Spread Analysis — Standalone
========================================
Three angles on buy/sell price divergence.
Standalone version with engine code embedded.
"""
...
# ============================================================
# Extended wallet: records ratio at each buy
# ============================================================

@dataclass
class SpreadWallet(CohortWallet):
    buy_ratios: List[float] = field(default_factory=list)  # sell/buy at buy time
    buy_stages: List[int]   = field(default_factory=list)  # stage at buy time


def make_spread_wallets(cohort_sizes, sol_per_wallet, rng):
    wallets = []
    wid = 0
    for cohort, n in cohort_sizes.items():
        for _ in range(n):
            sol = int(sol_per_wallet * rng.uniform(0.5, 1.5))
            w = SpreadWallet(wallet_id=wid, sol=sol, cohort=cohort)
            w.initial_sol = sol
            wallets.append(w)
            wid += 1
    return wallets
...
# ============================================================
# Instrumented engine: records ratio at every buy
# ============================================================

def run_spread_trial(cohort_sizes, n_steps, sol_per_wallet, seed):
    rng = random.Random(seed)
    ...
    if holders:
        ...
        for w in holders:
            w.tokens                  = per
            w.cost_basis              = per * init_price
            w.initial_tokens          = per
            w.initial_token_cost_basis = per * init_price
            w.entry_stages.append(1)
            w.entry_prices.append(init_price)
            # initial inventory: exclude from purchase ratio tracking
            # (ratio would be compared against Stage 3 price, which is misleading)
            w.buy_stages.append(2)

    # 1. Stage-entry snapshots
    stage_entry_ratios: Dict[int, List[float]] = defaultdict(list)

    # Record initial stage 3 entry ratio
    stage_entry_ratios[2].append(engine.sell_price() / engine.buy_price()
                                  if engine.buy_price() > 0 else 0)

    prev_stage = engine.state.current_stage

    # 2. Time-series: ratio every 500 steps
    time_series: List[Tuple[int, int, float]] = []   # (step, stage_human, ratio)
    ...
    for step in range(n_steps):
        ...
        if step % 500 == 0:
            ...
            sp = engine.sell_price()
            ratio = sp / bp if bp > 0 else 0
            time_series.append((step, human_stage(engine.state.current_stage), ratio))

        w = wallets[rng.randrange(len(wallets))]
        policy_fn = COHORT_POLICIES.get(w.cohort)
        if policy_fn:
            sp_before = engine.sell_price()
            bp_before = engine.buy_price()
            buys_before = len(w.entry_stages) if isinstance(w, SpreadWallet) else 0
            policy_fn(rng, engine, w, step)
            # if a buy happened, record sell/buy ratio at time of purchase
            if isinstance(w, SpreadWallet):
                buys_after = len(w.entry_stages)
                if buys_after > buys_before and bp_before > 0:
                    w.buy_ratios.append(sp_before / bp_before)
                    w.buy_stages.append(engine.state.current_stage)
            # detect stage advance
            if engine.state.current_stage != prev_stage:
                new_s = engine.state.current_stage
                bp_new = engine.buy_price()
                sp_new = engine.sell_price()
                stage_entry_ratios[new_s].append(sp_new / bp_new if bp_new > 0 else 0)
                prev_stage = engine.state.current_stage

        engine.stats.total_steps += 1

    return engine, wallets, stage_entry_ratios, time_series
...
def run_spread_mc(cohort_sizes, n_trials=200, n_steps=30_000,
                  sol_per_wallet=200*SOL, seed=42):

    # 1. Stage-entry ratios
    stage_entry: Dict[int, List[float]] = defaultdict(list)

    # 2. Time-series by stage
    ts_by_stage: Dict[int, List[float]] = defaultdict(list)

    # 3. Cohort buy ratios
    cohort_ratios: Dict[str, List[float]] = defaultdict(list)

    for trial in range(n_trials):
        if (trial + 1) % 50 == 0:
            print(f"    {trial+1}/{n_trials}...", flush=True)

        engine, wallets, entry_ratios, ts = run_spread_trial(
            cohort_sizes, n_steps, sol_per_wallet, seed + trial
        )

        for stage_idx, ratios in entry_ratios.items():
            stage_entry[stage_idx].extend(ratios)

        for (step, stage_h, ratio) in ts:
            ts_by_stage[stage_h].append(ratio)

        for w in wallets:
            for r, s in zip(w.buy_ratios, w.buy_stages):
                cohort_ratios[w.cohort].append(r)

    return {
        "stage_entry":   dict(stage_entry),
        "ts_by_stage":   dict(ts_by_stage),
        "cohort_ratios": dict(cohort_ratios),
    }
...
def print_spread_results(label, result):
    entry  = result["stage_entry"]
    ts     = result["ts_by_stage"]
    cohort = result["cohort_ratios"]
    ...
    # 1. Stage-entry ratio
    print(f"\n  1. sell/buy ratio at stage entry (lower = bigger spread):")
    print(f"  {'stage':>8}  {'avg ratio':>10}  {'downside%':>12}  {'trials':>7}")
    print(f"  {'-'*45}")
    PHI = (1 + 5**0.5) / 2
    for idx in sorted(entry.keys()):
        vals = entry[idx]
        avg  = sum(vals) / len(vals)
        ds   = (1 - avg * 0.995) * 100  # immediate downside
        print(f"  Stage {human_stage(idx):>2}     {avg:>10.4f}  {ds:>10.2f}%  {len(vals):>7}")

    # 2. Time-series
    print(f"\n  2. sell/buy ratio over time (by stage):")
    print(f"  {'at stage':>10}  {'avg ratio':>10}  {'immediate downside':>20}")
    print(f"  {'-'*44}")
    for sh in sorted(ts.keys()):
        vals = ts[sh]
        avg  = sum(vals) / len(vals)
        ds   = (1 - avg * 0.995) * 100
        bar  = '▓' * int(avg * 20)
        print(f"  Stage {sh:>2}      {avg:>10.4f}  {ds:>18.2f}%  {bar}")

    # 3. Cohort experience
    print(f"\n  3. sell/buy ratio at time of purchase (by cohort):")
    print(f"  {'cohort':>16}  {'avg ratio':>10}  {'downside%':>12}  {'n buys':>8}")
    print(f"  {'-'*52}")
    for c in sorted(cohort.keys()):
        vals = cohort[c]
        if not vals:
            continue
        avg = sum(vals) / len(vals)
        ds  = (1 - avg * 0.995) * 100
        print(f"  {c:>16}  {avg:>10.4f}  {ds:>10.2f}%  {len(vals):>8,}")

    print(f"\n  reference: 1/φ = {1/(1+5**0.5)*2:.4f} (theoretical floor ratio at high stages)")
    print(f"             1 - 0.995/φ = {(1-0.995/PHI)*100:.2f}% (theoretical max downside)")
...
if __name__ == "__main__":
    ...
    for key, scenario in SCENARIOS.items():
        print(f"\nRunning {scenario['label']} ({N_TRIALS} trials)...")
        result = run_spread_mc(
            cohort_sizes=scenario["cohort_sizes"],
            n_trials=N_TRIALS,
            n_steps=N_STEPS,
        )
        print_spread_results(scenario["label"], result)
